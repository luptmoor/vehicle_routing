...
def plot_sensitivity_boxplots(df, parameter="speed"):
    ...
    Saves all plots to the 'Figures/' directory.
    """
    plt.figure(figsize=(8, 6))
    sns.boxplot(x="Multiplier", y="Objective Value Change %", hue="Change Type", data=df, palette={"Decrease": "blue", "Increase": "red"})
    ...
    plt.figure(figsize=(8, 6))
    sns.boxplot(x="Multiplier", y="Total Distance Change %", hue="Change Type", data=df, palette={"Decrease": "blue", "Increase": "red"})
    plt.xlabel(f"{parameter.capitalize()} Multiplier")
...
def plot_sensitivity_scatter(df, parameter="speed"):
    ...
    """
    plt.figure(figsize=(8, 6))
    sns.scatterplot(x="Multiplier", y="Objective Value Change %", hue="Change Type", data=df, palette={"Decrease": "blue", "Increase": "red"})
    plt.xlabel(f"{parameter.capitalize()} Multiplier")
    plt.ylabel("Objective Value Change (%)")
    plt.title(f"Objective Value Change vs {parameter.capitalize()} Multiplier")
    plt.xticks(sorted(df["Multiplier"].unique()))  # Ensure x-ticks are only at multiplier values
    plt.grid(alpha=0.7)
    plt.savefig(f"Figures/{parameter}_objective_change_scatter.png", dpi=300)
    plt.show()

    plt.figure(figsize=(8, 6))
    sns.scatterplot(x="Multiplier", y="Total Distance Change %", hue="Change Type", data=df, palette={"Decrease": "blue", "Increase": "red"})
    plt.xlabel(f"{parameter.capitalize()} Multiplier")
    plt.ylabel("Total Distance Change (%)")
    plt.title(f"Total Distance Change vs {parameter.capitalize()} Multiplier")
    plt.xticks(sorted(df["Multiplier"].unique()))  # Ensure x-ticks are only at multiplier values
...
df_speed = compute_sensitivity_changes(N=9, M=5, trials=TRIALS, lower=LOWER, upper=UPPER, steps=STEPS, parameter="speed")
...
df_capacity = compute_sensitivity_changes(N=9, M=5, trials=100, lower=0.91, upper=1.09, steps=4, parameter="capacity")
